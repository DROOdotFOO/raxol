defmodule Raxol.UI.TextLayout.PrettyTest do
  ...
  pretty` dynamic program.

  This module is intentionally self-contained: it does NOT import
  `Raxol.UI.TextLayout` (owned by a concurrent agent). Where a greedy
  reference wrap is needed for comparison, a tiny private greedy
  ...
  defp greedy_wrap(words, width) do
    ...
    |> Enum.reduce([], fn word, lines ->
      case lines do
        ...
        [current | rest] ->
          candidate_width =
            TextMeasure.display_width(current) + 1 + TextMeasure.display_width(word)
  ...
  defp line_word_count(line), do: line |> String.split(" ", trim: true) |> length()
  ...
  describe "goldens" do
    test "classic raggedness case: pretty balances the orphan, greedy doesn't" do
      ...
      assert greedy == ["The quick brown", "fox jumps over", "the lazy dog", "today"]
      assert line_word_count(List.last(greedy)) == 1

      # Pretty pulls a second word down onto the last line instead.
      assert pretty == ["The quick", "brown fox jumps", "over the lazy", "dog today"]
  ...
  # ---------------------------------------------------------------------
  # Cost sanity: DP total cost strictly beats greedy's total cost
  # ---------------------------------------------------------------------
  ...
  # Bounded-width plain ASCII word generator: every word's display width is
  # guaranteed <= the paired container width, so the "single overlong
  # token" exception never triggers and every line must strictly respect
  # `width`.
