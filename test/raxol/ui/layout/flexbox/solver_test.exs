defmodule Raxol.UI.Layout.Flexbox.SolverTest do
  ...
  defp solve(items, container), do: Solver.resolve_flexible_lengths(items, container, :horizontal)
  ...
  describe "properties" do
      ...
      check all(
        ...
        items =
          for {base, grow, shrink, min, max} <- specs do
            item(base_size: base, grow: grow, shrink: shrink, min_main: min, max_main: max(min, max))
          end
    ...
    property "when growing with no max clamps, outer sizes exactly fill the container" do
      check all(
              bases <- StreamData.list_of(StreamData.integer(0..20), min_length: 1, max_length: 6),
