defmodule Raxol.UI.Components.MarkdownRendererTest do
  ...
  describe "render_with_builtin/2 (forced, EarmarkParser-independent)" do
    test "matches the default render_md output when EarmarkParser is unavailable" do
      # EarmarkParser is only a transitive :dev dep of ex_doc (see mix.exs
      # `{:ex_doc, "~> 0.38", only: :dev, runtime: false}`), so it is never
      # loaded in :test -- render_md/2 above already exercises this same
      # function. This test calls it directly (it's `@doc false`, public
      # for exactly this purpose) to make that coupling explicit and pin
      # the contract independent of environment.
