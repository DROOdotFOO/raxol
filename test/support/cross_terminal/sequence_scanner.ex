defmodule Raxol.Test.CrossTerminal.SequenceScanner do
  ...
  defp take_csi(<<char, rest::binary>>, acc), do: take_csi(rest, acc <> <<char>>)
  ...
  defp take_until_st(<<char, rest::binary>>, acc), do: take_until_st(rest, acc <> <<char>>)
  ...
  def capability({:csi, params, "m"}) do
    cond do
      String.contains?(params, "38;2;") or String.contains?(params, "48;2;") -> :truecolor
      String.contains?(params, "38;5;") or String.contains?(params, "48;5;") -> :color256
      true -> :basic
    end
  end
  ...
  @profiles %{
    ...
    xterm_256color: MapSet.new([:basic, :color256, :mouse, :alt_screen, :bracketed_paste, :osc]),
