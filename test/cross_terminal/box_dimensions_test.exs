defmodule Raxol.CrossTerminal.BoxDimensionsTest do
  @moduledoc """
  Regression tests for layout-engine box sizing (problems backlog L1/L2):
  `box style: %{width: N}` was rendered full-bleed and padding from the
  View DSL (normalized to a 4-tuple) was silently dropped.

  Uses the real playground TextInput demo at a large screen size — the
  original reproduction from the JetBrains-terminal screenshot.
  """
  ...
  test "box with explicit style width stays at that width on a wide screen" do
    ...
    border_lines =
      Enum.filter(lines, &(String.starts_with?(&1, "┌") or String.starts_with?(&1, "└")))
  ...
  test "box padding insets content from the border" do
    ...
    content_line = Enum.find(lines, &String.contains?(&1, "(type to enter text)"))
    ...
    content_idx = Enum.find_index(lines, &String.contains?(&1, "(type to enter text)"))
