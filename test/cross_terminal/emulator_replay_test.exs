defmodule Raxol.CrossTerminal.EmulatorReplayTest do
  @moduledoc """
  Characterization tests for the reference emulator via AnsiReplayer.

  These pin the CURRENT behavior of the `TerminalParser` + `Emulator`
  pipeline on the sequence families the renderer emits. Any refactor of
  the parser/emulator (problems backlog #1, #3, #17) must keep these green.
  """
  ...
  describe "wide characters" do
    test "CJK char occupies two cells" do
      ...
      assert first.char == "你"
      # placeholder cell behavior is pinned as-is; the invariant that matters:
      # the following narrow char must land at x=2, not x=1
      assert third.char == "a"
      refute second.char == "a"
    end
  end

  describe "mode 2026 (synchronized output) — documents problem backlog #1" do
    test "emulator currently does not track mode 2026 (pin until implemented)" do
      # We emit \e[?2026h in advanced_features.ex but the emulator has no
      # mode-table entry for it. This test documents the asymmetry: feeding
      # the sequence must at minimum not crash or corrupt the grid.
