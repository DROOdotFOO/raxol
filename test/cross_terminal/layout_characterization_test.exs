defmodule Raxol.CrossTerminal.LayoutCharacterizationTest do
  ...
  describe "flex_wrap" do
    ...
    test "style-based flex_wrap is honored by the live engine" do
      ...
      # wide, words 4 wide: two fit on line 0, third wraps. align-content
      # defaults to :stretch (CSS initial) on every flex entry path, so
      # the two 1-tall lines share the 10-tall container 5/5 and the
      # wrapped line starts at y=5. Output order is line-reversed
      # (pre-existing Wrapper accumulation quirk) - sort for stability.
      result =
