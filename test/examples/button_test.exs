defmodule Raxol.Examples.ButtonTest do
  ...
  describe "visual tests" do
    ...
    test "matches snapshot", context do
      ...
      # test/support/raxol/visual.ex lays out through the live
      # Raxol.UI.Layout.Engine pipeline, whose x/y/width/height element
      # shape matches what Raxol.UI.Renderer.render_to_cells/2 expects, so
      # this snapshot shows a real bordered button box with the "Button"
      # label.
    ...
    test "renders normal and disabled states for snapshot testing", context do
      ...
      # Same live-engine harness as button_submit above: these snapshots
      # show real "┌───┐ / │ Normal │ / └───┘" and "│ Disabled │" boxes.
