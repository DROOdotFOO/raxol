defmodule Raxol.Terminal.ANSI.InputParserTest do
  ...
  describe "unmapped terminal reports (render-storm regression)" do
    # Well-formed CSI sequences with no event mapping used to leak: the
    # parser skipped only the ESC byte and re-parsed the remaining bytes
    # as printable char key events. One volunteered secondary DA report
    # then became 10 keystrokes -- each dispatched to the app and each
    # triggering a full-frame repaint (and the leaked "c" toggled the
    # playground code panel). Terminal reports must produce ZERO events.
