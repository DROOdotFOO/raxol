defmodule Raxol.Terminal.RendererIntegrationTest do
  ...
  describe "{r, g, b} tuple colors (Raxol.UI.CellDim / plain UI-stack RGB)" do
    # `{r, g, b}` integer tuples are the RGB shape used throughout the UI
    # layer (ElementRenderer/StyleProcessor styles, hand-authored
    # `bg: {r, g, b}` in app/component code, and `Raxol.UI.CellDim`'s
    # modal-backdrop dimming output). Regression coverage for the bug
    # where `resolve_fg_ansi/2`/`resolve_bg_ansi/2` only matched
    # `%{r:, g:, b:}`-shaped maps: a plain tuple silently fell through to
    # the `nil` catch-all, so no color escape code was emitted at all and
    # the cell inherited whatever SGR state preceded it -- observed as
    # dimmed content losing its background/hue entirely behind an open
    # modal.
