  ...
  defp parse_csi_tilde_one(data) do
    case parse_csi_params(data) do
      ...
      {_params, final_byte, rest} when is_integer(final_byte) ->
        # Well-formed CSI sequence we have no event mapping for: terminal
        # reports like a secondary DA reply (`CSI > 0;282;0 c`), a cursor
        # position report (`CSI 42;120 R`), a DECRQM response
        # (`CSI ? 2026;2 $y`), etc. Consume the whole sequence and emit
        # nothing. Returning `{nil, data}` here would make parse_loop skip
        # only the ESC and re-parse the remaining bytes as printable char
        # key events -- one volunteered terminal report then becomes ~10
        # keystrokes (and ~10 render cycles) of garbage.
        {:consumed, rest}
