defmodule Raxol.Payments.LedgerSubscribeTest do
  ...
  test "subscribe/2 delivers ledger entries from try_spend", %{ledger: ledger, policy: policy} do
    :ok = Ledger.subscribe(ledger)
    :ok = Ledger.try_spend(ledger, :agent_a, Decimal.new("0.10"), policy, %{protocol: "x402"})
  ...
  test "subscribe/2 delivers entries from record_spend (cast path)", %{ledger: ledger} do
    :ok = Ledger.subscribe(ledger)
    :ok = Ledger.record_spend(ledger, :agent_b, Decimal.new("0.05"), %{to: "0xabc"})
  ...
  test "agent_id filter only delivers matching entries", %{ledger: ledger, policy: policy} do
  ...
  test "subscriber DOWN cleans up state without crashing the ledger", %{
    ...
    assert :ok = Ledger.try_spend(ledger, :post_down, Decimal.new("0.01"), policy)
