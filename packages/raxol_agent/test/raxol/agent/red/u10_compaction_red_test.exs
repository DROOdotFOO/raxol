defmodule Raxol.Agent.Red.U10CompactionRedTest do
  ...
  These POSITIVE contours run against the REAL `Raxol.Agent.Compaction`, which
  is a `:not_implemented` skeleton until U9 (checkpoint pointer records) + MS
  (model-snapshot contract) land. They are therefore RED by construction and
  carry `@moduletag :harness_red` (excluded from CI via test_helper). Run them
  on purpose with `mix test --only harness_red`.
  ...
  setup do
    base = Path.join(System.tmp_dir!(), "u10_red_#{System.unique_integer([:positive])}")
    File.mkdir_p!(base)
  ...
  test "P1 — compaction is one artifact: a checkpoint{reason: compaction}, one dense offset via the real Writer",
    ...
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 2}, %{"op" => "note", "text" => "a"}])
  ...
  test "P2 — compact then resume equals a direct full fold on the persistent slice (real MS codec)",
    ...
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 2}, %{"op" => "note", "text" => "x"}])
    ...
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 5}, %{"op" => "note", "text" => "y"}])
    ...
    assert resumed == expected, "resume must equal fold(0..now) on the persistent slice (P-JS4)"
  ...
  test "P3 — journal bytes below the checkpoint offset are byte-identical before/after compaction (FI-7)",
    ...
    S.seed_events!(journal, for(n <- 1..16, do: %{"op" => "incr", "amount" => n}))
  ...
  test "P4 — every non-persistable field omitted from the snapshot appears in the manifest",
    ...
    model =
      ...
      |> Map.merge(%{"_conn_pid" => "#PID<0.123.0>", "_secret_token" => "sk-live-xyz"})
    ...
    refute "_secret_token" in Map.keys(slice), "secrets never reach the snapshot slice"
  ...
  test "P5 — a second compaction appends a fresh checkpoint; the older stays healthy, resume selects the newest",
    ...
    assert {:ok, c1} = Compaction.compact(journal, model: S.fold(S.records_of(journal)))

    S.seed_events!(journal, [%{"op" => "incr", "amount" => 3}])
    assert {:ok, c2} = Compaction.compact(journal, model: S.fold(S.records_of(journal)))
    ...
    assert sel == c2.checkpoint_offset, "resume must select the LATEST healthy checkpoint"
  ...
  test "P6 — resume picks the latest healthy checkpoint; a corrupt newest falls back WITH the typed error surfaced",
    ...
    assert sel == c1.checkpoint_offset, "resume must fall back to the previous healthy checkpoint"
...
defmodule Raxol.Agent.Red.U10CompactionControlsTest do
  ...
  alias Raxol.Agent.Journal.FileStore
  alias Raxol.Agent.Red.Support.U10Compaction, as: S
  alias Raxol.Agent.Red.Support.U10Compaction.{FaultCounter, Injectors, RefCompactor}

  setup do
    base = Path.join(System.tmp_dir!(), "u10_ctl_#{System.unique_integer([:positive])}")
    File.mkdir_p!(base)
    on_exit(fn -> File.rm_rf(base) end)
    {:ok, base: base}
  end
  ...
  test "dead injector (a): a separate \"compaction\" kind fails one_artifact?; the reference passes",
    ...
    {:ok, ref_result} = RefCompactor.compact(ref_j, model: S.fold(S.records_of(ref_j)))
    ...
    {:ok, bad_result} =
      Injectors.separate_kind(bad_j, model: S.fold(S.records_of(bad_j)), fault_counter: fc)
  ...
  test "dead injector (c): a lossy summarizer (empty manifest) fails omission_accounted?; the reference accounts every field",
    ...
    assert S.omission_accounted?(ref_model, S.persist(ref_model), ref_result.manifest)
    FileStore.close(ref_j)

    {bad_j, _bad_dir} = seed_one(base)
    bad_model = S.fold(S.records_of(bad_j)) |> Map.merge(volatile)
    {:ok, bad_result} = Injectors.lossy_summarizer(bad_j, model: bad_model, fault_counter: fc)

    refute S.omission_accounted?(bad_model, S.persist(bad_model), bad_result.manifest),
  ...
  test "dead injector (d): a silent fallback on a corrupt newest fails resume_selection_surfaced?; the reference surfaces the typed error",
    ...
    assert sel < corrupt_offset, "reference falls back to the previous healthy checkpoint"
  ...
  defp two_checkpoints(base) do
    {journal, _session, dir} = S.fresh_session(base)
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 1}])
    {:ok, _c1} = RefCompactor.compact(journal, model: S.fold(S.records_of(journal)))
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 1}])
    {:ok, _c2} = RefCompactor.compact(journal, model: S.fold(S.records_of(journal)))
    {journal, dir}
  end
  ...
  test "control: reference compact+resume equals the full fold; a resume that drops post-checkpoint events does NOT",
    ...
    {journal, _session, _dir} = S.fresh_session(base)
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 2}])
    {:ok, _c} = RefCompactor.compact(journal, model: S.fold(S.records_of(journal)))
    S.seed_events!(journal, [%{"op" => "incr", "amount" => 5}, %{"op" => "note", "text" => "z"}])

    {:ok, resumed, _} = RefCompactor.resume(journal, [])
    full = S.persist(S.fold(S.records_of(journal)))
    assert resumed == full, "correct resume equals fold(0..now) on the persistent slice"
  ...
  test "control: repeated compaction — both checkpoints healthy, resume selects newest, restore-from-each is consistent",
    ...
    c1 = %{snapshot_ref: c1_rec["snapshot_ref"], snapshot_hash: c1_rec["snapshot_hash"]}
    c2 = %{snapshot_ref: c2_rec["snapshot_ref"], snapshot_hash: c2_rec["snapshot_hash"]}

    assert S.checkpoint_healthy?(dir, c1)
    assert S.checkpoint_healthy?(dir, c2)

    {:ok, from_latest, %{selected_offset: sel}} = RefCompactor.resume(journal, [])
    ...
    assert from_older == from_latest, "restore-from-each converges on the current tip state"
