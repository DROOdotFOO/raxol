defmodule Raxol.Agent.Red.SteerFaults do
  ...
  the three U6 steer dead injectors. A control arms every site, records each one
  as it confirms the matching contour is caught, and calls `assert_all_fired!/2`
  so an injector that silently stopped injecting fails the control instead of
  passing quietly.
  """

  @sites [:skip_cas, :journal_before_cas, :drop_dedup, :in_memory_only_dedup]

  @doc "All named U6 steer fault sites (the three dead injectors)."
  def sites, do: @sites
