defmodule Raxol.ACP.Relay.OnchainBroadcaster do
  @moduledoc """
  Real `Raxol.Payments.Relay.Broadcaster` backed by raxol_acp's EVM transaction
  stack. It funds a Relay deposit by broadcasting an ERC-20 `transfer` to the
  Riddler deposit address on the source EVM chain.

  This lives in raxol_acp (not raxol_payments) because that is where the proven
  EIP-1559 signing / RLP / nonce / JSON-RPC stack lives, and raxol_acp depends on
  raxol_payments. raxol_payments stays free of fund-moving transaction code; the
  action injects this module.

  ## Wiring

  Supply a `Raxol.ACP.ProviderAdapter` (a `JSONRPC` adapter for production) either
  per call via the `:provider` param, or once at startup:

      provider = Raxol.ACP.ProviderAdapter.JSONRPC.new(
        chains: %{8453 => System.fetch_env!("BASE_RPC_URL")},
        private_key: agent_eoa_private_key
      )

      Raxol.ACP.Relay.OnchainBroadcaster.configure(provider)
