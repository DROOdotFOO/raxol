defmodule Raxol.ACP.Xochi.CapacityLedger do
  @moduledoc """
  Rolling per-destination reservation ledger for `Raxol.ACP.Xochi.TransferOffering`.
  ...
  capacity from `config :raxol_acp, :destination_capacity` (or `load/2`, e.g. from
  the `raxol_acp.derive_caps` mix task). When the process is not running, the
  ...
  capacity is unbounded.
      ...
      # supervision
      children = [Raxol.ACP.Xochi.CapacityLedger]
  """
  ...
  def init(opts) do
    capacity =
      Keyword.get_lazy(opts, :capacity, fn ->
        Application.get_env(:raxol_acp, :destination_capacity, %{})
      end)
