defmodule Raxol.ACP.Xochi.CapacityGate do
  @moduledoc """
  Supervises the offering's liquidity guardrail: `Raxol.ACP.Xochi.CapacityLedger`
  (aggregate reservation state) and `Raxol.ACP.Xochi.CapacityRefresher` (periodic
  live re-derivation). `:one_for_one`, so a refresher hiccup does not reset the
  ledger's reservations and vice versa.

  Added to the seller supervision tree by `Raxol.ACP.Seller.Supervisor` when
  `config :raxol_acp, capacity_gate_enabled: true`. The ledger seeds from
  ...
  alias Raxol.ACP.Xochi.{CapacityLedger, CapacityRefresher}
  ...
  def init(opts) do
    refresher_opts =
      ...
      |> Keyword.put_new(
        :interval_ms,
        Application.get_env(:raxol_acp, :capacity_refresh_interval_ms, 900_000)
      )
