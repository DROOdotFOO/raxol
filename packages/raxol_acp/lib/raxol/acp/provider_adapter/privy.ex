defmodule Raxol.ACP.ProviderAdapter.Privy do
  @moduledoc """
  `Raxol.ACP.ProviderAdapter` for a Virtuals-managed (Privy + Alchemy) agent
  ...
  `Raxol.ACP.ProviderAdapter.JSONRPC` (which signs locally with `ExSecp256k1`).
  ...
  (`priv/signer_sidecar/server.mjs`, supervised by `Raxol.ACP.SignerSidecar`),
  ...
  `Raxol.ACP.Onchain.RPC` -- identical to the JSONRPC adapter -- rather than
  through the sidecar.

  ## Construction

      Raxol.ACP.ProviderAdapter.Privy.new(
  ...
  See `Raxol.ACP.ProviderAdapter.SCA` for the other no-local-EOA adapter.
  """

  @behaviour Raxol.ACP.ProviderAdapter

  alias Raxol.ACP.{ABI, Onchain.Hex, Onchain.RPC}
