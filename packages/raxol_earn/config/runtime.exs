...
# Runtime (boot-time) config for the raxol_earn release. Two modes share this file:
#
...
if config_env() == :prod do
  ...
  config :raxol_earn, xochi_solver_enabled: solver_enabled?
  ...
  if solver_enabled? and System.get_env("RELEASE_DISTRIBUTION", "none") not in ["none", ""] do
    raise """
    raxol_earn solver mode forbids Erlang distribution: RELEASE_DISTRIBUTION must be \
  ...
  if solver_enabled? and not delegated_signing? and
    ...
    raise """
    raxol_earn legacy-EOA solver mode requires RAXOL_ACP_RPC_URL (a private Base RPC). \
