defmodule Raxol.Earn.ProviderAdapter.JsonrpcNonceTest do
  @moduledoc """
  Hermetic proof that `Raxol.Earn.ProviderAdapter.JSONRPC` serializes nonce
  assignment through `Raxol.Earn.Wallet.NonceServer`, so concurrent
  `send_calls/3` for the same EOA never sign the same nonce.

  The RPC is stubbed with a `:plug` (picked up from the `:raxol_earn, :rpc`
  ...
  setup do
    ...
    Application.put_env(:raxol_earn, :rpc, plug: plug(table))
    on_exit(fn -> Application.delete_env(:raxol_earn, :rpc) end)
