defmodule Raxol.Earn.ProviderAdapter.SCATest do
  @moduledoc """
  Stubbed coverage for `Raxol.Earn.ProviderAdapter.SCA`.
  ...
  `Raxol.Earn.Wallet.SCA.BundlerHarnessTest` (`:live_bundler`).
  """
  ...
  alias Raxol.Earn.ProviderAdapter.SCA
  alias Raxol.Earn.Wallet.SCA.ModularAccount
  ...
  defmodule SCAWallet do
    use Raxol.Earn.Wallet.SCA,
      ...
      chain_id: 8453,
      signer: Raxol.Earn.ProviderAdapter.SCATest.SessionKey,
      signer_entity_id: 1,
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:raxol_earn, :rpc)
    end)
  ...
  defp acp_call do
    %{
      to: @acp_core,
      data: Raxol.Earn.ABI.encode_call("claimBudget(uint256)", [{"uint256", 42}]),
      value: 0
    }
  end
  ...
  # legs are picked up via the :raxol_earn :rpc app config; the bundler /
  ...
  defp install_stub(events, opts) do
    ...
    Application.put_env(:raxol_earn, :rpc, url: @rpc_url, plug: plug)
    plug
  end
