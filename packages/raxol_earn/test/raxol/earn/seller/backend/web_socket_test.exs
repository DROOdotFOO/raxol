defmodule Raxol.Earn.Seller.Backend.WebSocketTest do
  @moduledoc """
  Integration test for `Raxol.Earn.Seller.Backend.WebSocket` against a
  real Cowboy-backed Socket.IO v4 server (see
  `Raxol.Earn.TestSupport.SocketIOTestServer`).
  ...
  alias Raxol.Earn.Seller.Backend.WebSocket
  alias Raxol.Earn.Seller.Backend.WebSocket.Connection
  alias Raxol.Earn.TestSupport.SocketIOTestServer
  ...
  defp start_backend(url, opts \\ []) do
    Application.put_env(:raxol_earn, :seller_backend_url, url)
    Application.put_env(:raxol_earn, :seller_backend_auth, Keyword.get(opts, :auth))

    on_exit(fn ->
      Application.delete_env(:raxol_earn, :seller_backend_url)
      Application.delete_env(:raxol_earn, :seller_backend_auth)
    end)
  ...
  describe "resolve_url/1 precedence" do
    setup do
      on_exit(fn ->
        Application.delete_env(:raxol_earn, :seller_backend_url)
        Application.delete_env(:raxol_earn, :chain)
      end)
    ...
    test "explicit opt wins over env and chain" do
      Application.put_env(:raxol_earn, :seller_backend_url, "https://env.example")
      Application.put_env(:raxol_earn, :chain, :sepolia)
    ...
    test "chain.mainnet acp_socket_url used when env unset" do
      Application.delete_env(:raxol_earn, :seller_backend_url)
      Application.put_env(:raxol_earn, :chain, :mainnet)
    ...
    test "unknown chain falls back to hardcoded mainnet URL" do
      Application.delete_env(:raxol_earn, :seller_backend_url)
      Application.put_env(:raxol_earn, :chain, :goerli)
      assert WebSocket.resolve_url([]) == "https://acpx.virtuals.io"
    end
  end
end
