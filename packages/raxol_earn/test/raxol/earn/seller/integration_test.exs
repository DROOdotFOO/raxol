defmodule Raxol.Earn.Seller.IntegrationTest do
  @moduledoc """
  ...
  setup do
    ...
    Application.put_env(:raxol_earn, :seller_provider_adapter, adapter)
    Application.put_env(:raxol_earn, :seller_chain_id, @chain)
    Application.put_env(:raxol_earn, :seller_acp_core_address, @core)
  ...
  test "backend events drive a job from offer to completion via hook calls", %{adapter: adapter} do
    ...
    assert selectors_sent(adapter) == [
             Raxol.Earn.ABI.function_selector("setBudget(uint256,uint256,bytes)"),
             Raxol.Earn.ABI.function_selector("submit(uint256,bytes32,bytes)")
           ]
  end
