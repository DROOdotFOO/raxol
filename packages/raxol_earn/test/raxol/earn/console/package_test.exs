defmodule Raxol.Earn.Console.PackageTest do
  # async: false -- the round-trip test flips the inference module via app env.
  use ExUnit.Case, async: false

  alias Raxol.Earn.Console.{Generator, Package, Spec}

  describe "parse/1 <-> Generator round-trip" do
    setup do
      prev_inf = Application.get_env(:raxol_earn, :console_inference)
      prev_static = Application.get_env(:raxol_earn, :console_inference_static)
      ...
      Application.put_env(:raxol_earn, :console_inference,
        module: Raxol.Earn.Console.Inference.Static
      )
  ...
  defp restore(key, nil), do: Application.delete_env(:raxol_earn, key)
  defp restore(key, val), do: Application.put_env(:raxol_earn, key, val)
end
