defmodule Raxol.Earn.Onchain.TransactionTest do
  use ExUnit.Case, async: true

  alias Raxol.Earn.Onchain.Transaction
  ...
  describe "serialize/2" do
    ...
    test "includes signature components in the encoded list" do
      ...
      unsigned = ExKeccak.hash_256(<<0x02>> <> Raxol.Earn.Onchain.Rlp.encode(unsigned_fields(tx)))
    ...
    test "signing_hash matches keccak of (0x02 || rlp(unsigned_fields))" do
      tx = base_tx()
      manual = ExKeccak.hash_256(<<0x02>> <> Raxol.Earn.Onchain.Rlp.encode(unsigned_fields(tx)))
