defmodule Raxol.Earn.Xochi.JourneyTest do
  @moduledoc """
  ...
  The happy and rejection journeys read as `Raxol.Earn.Test.Scenario` pipes; the
  ...
  describe "destination inventory exhaustion" do
    test "settles the fillable subset, then quotes can_solve:false once inventory is gone" do
      ...
      {:ok, fake} =
        Raxol.Earn.TestSupport.FakeXochi.start_link(inventory: %{{to, dst} => @amount_int * 2})

      cfg = Raxol.Earn.TestSupport.FakeXochi.config(fake)
      ...
      assert Raxol.Earn.TestSupport.FakeXochi.inventory(fake, to, dst) == 0
    end
  end
