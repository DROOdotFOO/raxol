defmodule Raxol.Earn.Xochi.LiveOrderTest do
  @moduledoc """
  ...
  if System.get_env("XOCHI_ORDER_LIVE_URL") && System.get_env("XOCHI_ORDER_LIVE_KEY") do
    ...
    setup do
      ...
      ensure_registered()
      on_exit(fn -> Application.delete_env(:raxol_earn, :xochi_transfer_settler) end)
    ...
    defp maybe_enable_corridor_allowlist do
      if System.get_env("XOCHI_ORDER_STABLECOIN_ALLOWLIST") == "true" do
        prior = Application.get_env(:raxol_earn, :stablecoin_corridors_only)
    ...
    defp restore_acp_env(key, nil), do: Application.delete_env(:raxol_earn, key)
    defp restore_acp_env(key, value), do: Application.put_env(:raxol_earn, key, value)
