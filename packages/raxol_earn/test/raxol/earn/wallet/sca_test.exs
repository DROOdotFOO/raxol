defmodule Raxol.Earn.Wallet.SCATest do
  use ExUnit.Case, async: false

  alias Raxol.Earn.Wallet.SCA.{ModularAccount, UserOp}
  ...
  describe "provisioning helpers" do
    test "predicted_address/1 derives the SMA address from the session key" do
      expected = Raxol.Earn.Wallet.SCA.Provisioning.predict_address(SessionKey.address(), 0)
    ...
    test "deploy_init_code/1 is factory ++ createSemiModularAccount(session_key, salt)" do
      expected = Raxol.Earn.Wallet.SCA.Provisioning.deploy_init_code(SessionKey.address(), 0)
    ...
    test "own_session_key_install_calldata/1 registers the session key at its entity" do
      expected =
        Raxol.Earn.Wallet.SCA.Provisioning.install_session_key_calldata(SessionKey.address(), 1)
