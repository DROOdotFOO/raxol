defmodule Raxol.Earn.Test.Scenario do
  @moduledoc """
  A pipe-friendly harness for ACP cross-chain settlement journeys, over
  `Raxol.Earn.TestSupport.FakeXochi`. Drives the real client -> protocol ->
  ...
  the `assert_*` steps return it, so a whole user journey reads as one pipe.
      ...
      import Raxol.Earn.Test.Scenario
  ...
  - `:destination_caps`, `:closed_origins` -- set the offering's liquidity gate
    (`:raxol_earn` app env, restored on exit): a per-`{dst_chain, dst_token}` max
  ...
  alias Raxol.Earn.Xochi.TransferOffering
  ...
  # Set a `:raxol_earn` gate env (`:destination_caps` / `:closed_origins`) from the
  ...
  defp put_gate_env(key, opts) do
    case Keyword.fetch(opts, key) do
      ...
      {:ok, value} ->
        prior = Application.get_env(:raxol_earn, key)
        Application.put_env(:raxol_earn, key, value)

        fn ->
          case prior do
            nil -> Application.delete_env(:raxol_earn, key)
            _ -> Application.put_env(:raxol_earn, key, prior)
          end
        end
    end
  end
