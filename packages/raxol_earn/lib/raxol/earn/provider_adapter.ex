defmodule Raxol.Earn.ProviderAdapter do
  @moduledoc """
  ...
  - `Raxol.Earn.ProviderAdapter.Mock` -- in-process; records
    every call and returns canned responses. Used by `Raxol.Earn.Agent`
    and `Raxol.Earn.HookClient` tests so the full v2 lifecycle can be
    exercised without an RPC or a wallet.
  - `Raxol.Earn.ProviderAdapter.SCA` (not yet implemented) -- production:
    wraps `Raxol.Earn.Wallet.SCA` so calls flow through the
  ...
  An adapter is a map carrying the implementing module and its config:
      ...
      %{adapter: Raxol.Earn.ProviderAdapter.Mock, config: %{table: ...}}
  ...
  matches the pattern used by `Raxol.Earn.Transport` and
  `Raxol.Earn.JobApi`.
