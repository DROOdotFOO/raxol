defmodule Raxol.Earn.JobSession do
  @moduledoc """
  Event-driven session for a single ACP v2 job.

  Replaces v1's `Raxol.Earn.Job.Server` for the v2 model. One GenServer
  per active job, registered under `Raxol.Earn.JobSession.Registry` by
  ...
  `{Raxol.Earn.JobSession, {chain_id, job_id}, entry}`.

  ## Lifecycle

      {:ok, _pid} = Raxol.Earn.JobSession.start_link(
        chain_id: 8453,
        job_id: "job-42",
        role: :provider
      )

      :ok = Raxol.Earn.JobSession.subscribe(server)

      {:ok, :budget_set} = Raxol.Earn.JobSession.set_budget(server, asset_token)
      # ... time passes; client funds ...
      {:ok, :submitted} = Raxol.Earn.JobSession.submit(server, deliverable)
      # evaluator's process:
      {:ok, :completed} = Raxol.Earn.JobSession.complete(server, "looks good")
  ...
  Actions are gated through `Raxol.Earn.JobSession.Tools.allowed?/3`.
  ...
  `FundTransferHook` etc. lives behind `Raxol.Earn.ProviderAdapter`:
  ...
  alias Raxol.Earn.JobSession.{Registry, Status, Tools}
