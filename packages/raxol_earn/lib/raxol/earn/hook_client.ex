defmodule Raxol.Earn.HookClient do
  @moduledoc """
  High-level wrapper around `Raxol.Earn.ProviderAdapter` for the v2 ACP
  Core hook calls.
  ...
  All of these route through whichever `Raxol.Earn.ProviderAdapter` the
  ...
  caller usually reads it from `Raxol.Earn.Chain.mainnet/0`.
  """
  @spec set_budget(
  ...
  defp submit_single(adapter, chain_id, to, signature, args) do
    data = Raxol.Earn.ABI.encode_call(signature, args)
  ...
  # `Raxol.Earn.ABI.encode_bytes32` accepts hex (with or without 0x);
