defmodule Raxol.Earn.JobIdResolver do
  @moduledoc """
  Behaviour for resolving the on-chain `jobId` that `createJob` assigns.

  `Raxol.Earn.HookClient.create_job/4` returns only the transaction hash --
  the assigned `jobId` is emitted in a `JobCreated` event log. A BUYER
  (`Raxol.Earn.JobSession.Client`) originates the job, so unlike the seller it
  must resolve that id before it can `fund`. This seam keeps that resolution
  injectable (like `Raxol.Earn.ProviderAdapter` / `Raxol.Earn.JobApi`), so it is
  ...
  alias Raxol.Earn.{JobApi, ProviderAdapter}
