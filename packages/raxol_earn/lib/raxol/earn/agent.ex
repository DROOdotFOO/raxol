defmodule Raxol.Earn.Agent do
  @moduledoc """
  Orchestrator for a v2 raxol_earn agent.

  Mirrors `AcpAgent` in `acp-node-v2`. Owns three injected dependencies:

  - **Provider** (`Raxol.Earn.ProviderAdapter`) -- the chain-facing client
    that submits on-chain hook calls. Any module/struct implementing the
    behaviour is accepted.
  - **Transport** (`Raxol.Earn.Transport`) -- the SSE chat stream + REST
    messaging. `Raxol.Earn.Transport.Mock` for tests; `Raxol.Earn.Transport.SSE`
    for production.
  - **JobApi** (`Raxol.Earn.JobApi`) -- the REST discovery API.

  ## Lifecycle

      {:ok, agent} = Raxol.Earn.Agent.start_link(
        provider: my_provider,
        transport: Raxol.Earn.Transport.SSE.new(network: :mainnet),
        api: Raxol.Earn.JobApi.HTTP.new(network: :mainnet),
        wallet_address: "0x...",
        supported_chain_ids: [8453]
      )

      :ok = Raxol.Earn.Agent.subscribe(agent)
      :ok = Raxol.Earn.Agent.start_stream(agent)
  ...
  2. Resolves the matching `Raxol.Earn.JobSession` (starting one if it
     doesn't exist yet) via `Raxol.Earn.JobSession.Supervisor`.
  3. Decodes the entry into the canonical `Raxol.Earn.JobSession.entry()`
     shape and threads it into the session.
  4. Forwards the entry to every subscribed caller as
     `{Raxol.Earn.Agent, agent_pid, session_pid, entry}`.
  ...
  alias Raxol.Earn.{Event, JobApi, JobSession, Transport}
  ...
  - `:transport` -- a `Raxol.Earn.Transport.t()`.
  - `:api` -- a `Raxol.Earn.JobApi.t()`.
  ...
  Used by the reattach path (`Raxol.Earn.Xochi.SolverAgent`) to recover a job's
