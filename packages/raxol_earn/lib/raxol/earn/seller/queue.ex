defmodule Raxol.Earn.Seller.Queue do
  @moduledoc """
  Routes ACP backend events to `Raxol.Earn.JobSession`s, driving each as the
  seller PROVIDER via `Raxol.Earn.JobSession.Provider`.

  The Queue is a single GenServer that owns the dispatch policy for the seller.
  Backend events arrive via `dispatch/1` (called by `Raxol.Earn.Seller.Runtime`
  ...
  `Raxol.Earn.HookClient` + the configured `Raxol.Earn.ProviderAdapter`, and
  ...
  ## Configuration
      ...
      config :raxol_earn,
        ...
        seller_provider_adapter: adapter,         # a Raxol.Earn.ProviderAdapter (required to write on-chain)
  ...
  - `[:raxol, :earn, :seller, :queue, :dispatched]` -- successful dispatch.
    Metadata: `%{type, job_id, offering}`.
  - `[:raxol, :earn, :seller, :queue, :dropped]` -- event dropped. Metadata:
  ...
  alias Raxol.Earn.{Chain, JobSession}
  ...
  defp read_defaults do
    %{
      seller_address: Application.get_env(:raxol_earn, :seller_address),
      max_active_jobs: Application.get_env(:raxol_earn, :seller_max_active_jobs, 100),
      provider_adapter: Application.get_env(:raxol_earn, :seller_provider_adapter),
      checkpoint: Raxol.Earn.Checkpoint.store(),
      job_api: seller_job_api(),
      chain_id: Application.get_env(:raxol_earn, :seller_chain_id, 8453),
      acp_core_address:
        Application.get_env(:raxol_earn, :seller_acp_core_address) ||
  ...
  # (forwarded to `Raxol.Earn.JobApi.HTTP.new/1`: `auth:`, `server_url:`,
  # `chain_ids:`). nil disables OOB deliverable posting here and resync boot
  # drain in `Raxol.Earn.Seller.Resync`.
  defp seller_job_api do
    case Application.get_env(:raxol_earn, :seller_job_api_opts) do
      nil -> nil
      opts when is_list(opts) -> Raxol.Earn.JobApi.HTTP.new(opts)
    end
  end
  ...
  defp maybe_post_deliverable(%{job_api: api} = defaults, job_id, %{deliverable: deliverable}) do
    case Raxol.Earn.JobApi.post_deliverable(api, defaults.chain_id, job_id, deliverable) do
      :ok -> :ok
  ...
  defp emit(suffix, metadata) do
    :telemetry.execute([:raxol, :earn, :seller, :queue, suffix], %{}, metadata)
  end
end
