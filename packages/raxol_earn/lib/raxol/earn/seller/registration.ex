defmodule Raxol.Earn.Seller.Registration do
  @moduledoc """
  ...
  that Virtuals has not yet published are isolated behind injectable seams:
    ...
    * the Service Registry contract address -- gated by
      `Raxol.Earn.Chain.require_service_registry/1` (nil until confirmed, so this
      fails closed with `{:error, :service_registry_not_configured}`);
    * the off-chain registration call -- `Raxol.Earn.JobApi.register_agent/2`
  ...
  registration variant would slot in as `Raxol.Earn.HookClient.register_agent/_`
  ...
  ever fail closed, so wiring it into `Raxol.Earn.Seller.Supervisor` waits on the
  address.
  """
