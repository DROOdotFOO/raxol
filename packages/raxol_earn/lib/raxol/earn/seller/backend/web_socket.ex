defmodule Raxol.Earn.Seller.Backend.WebSocket do
  @moduledoc """
  Live `Raxol.Earn.Seller.Backend` implementation backed by a Socket.IO
  v4 connection to the Virtuals ACP server.

  Owns a single `Raxol.Earn.Seller.Backend.WebSocket.Connection`
  ...
  ## Configuration
      ...
      config :raxol_earn,
        chain: :mainnet,
        seller_backend: Raxol.Earn.Seller.Backend.WebSocket,
        seller_backend_auth: %{walletAddress: "0x..."}

  URL resolution order (first match wins):

  1. Explicit `:url` opt to `start_link/1`
  2. `:raxol_earn, :seller_backend_url` application env
  3. `acp_socket_url` field on the configured chain
     (`Raxol.Earn.Chain.mainnet/0` -> `https://acpx.virtuals.io`,
     `Raxol.Earn.Chain.sepolia/0` -> `https://acpx.virtuals.gg`)
  ...
  Only the fields `Raxol.Earn.Seller.Queue` reads off the event are
  ...
  @behaviour Raxol.Earn.Seller.Backend
  ...
  @impl Raxol.Earn.Seller.Backend
  ...
  def init(opts) do
    ...
    auth = Keyword.get(opts, :auth) || Application.get_env(:raxol_earn, :seller_backend_auth)
  ...
  def resolve_url(opts) do
    cond do
      ...
      url = Application.get_env(:raxol_earn, :seller_backend_url) -> url
  ...
  def chain_socket_url do
    case Application.get_env(:raxol_earn, :chain, :mainnet) do
