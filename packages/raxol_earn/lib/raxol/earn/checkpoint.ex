defmodule Raxol.Earn.Checkpoint do
  @moduledoc """
  Config plumbing for the durable idempotency store the seller's
  `Raxol.Earn.JobSession.Provider` uses to survive crashes between an offering
  ...
  the per-job step keys.
      ...
      config :raxol_earn,
        checkpoint: {:ets, :raxol_earn_checkpoint},   # dev/test: supervised named table
  ...
  - `{:ets, name}` — a named public ETS table owned by
    `Raxol.Earn.Checkpoint.Owner`, which `Raxol.Earn.Seller.Supervisor` starts
  ...
  paired with `Raxol.Earn.Seller.Resync` (which re-reads the authoritative phase
  ...
  def required?, do: Application.get_env(:raxol_earn, :require_checkpoint, false)
  ...
  `Raxol.Earn.JobSession.Client`) rather than using per-step keys.
  """
