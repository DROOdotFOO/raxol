defmodule Raxol.Earn.Auth do
  ...
  2. Sign via the configured `Raxol.Earn.ProviderAdapter`.
  ...
  ## Usage
      ...
      {:ok, auth} =
        Raxol.Earn.Auth.start_link(
          provider: my_provider,
          server_url: "https://api-dev.acp.virtuals.io",
          chain_id: 8453
        )

      {:ok, token} = Raxol.Earn.Auth.token(auth)
      # ... use token in Authorization: Bearer ...

      :ok = Raxol.Earn.Auth.invalidate(auth)
  ...
  - `:provider` -- a `Raxol.Earn.ProviderAdapter.adapter()` used for
