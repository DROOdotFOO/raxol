defmodule Raxol.Earn.Onchain.RPC do
  @moduledoc """
  Minimal Ethereum JSON-RPC 2.0 client for the methods the on-chain
  write paths (`Raxol.Earn.ProviderAdapter.JSONRPC`,
  `Raxol.Earn.ProviderAdapter.SCA`, `Raxol.Earn.Wallet.SCA.Provisioner`)
  need.
  ...
  `Application.get_env(:raxol_earn, :rpc, ...)`. Callers can also pass a
  ...
  def client(opts \\ []) do
    config = Application.get_env(:raxol_earn, :rpc, [])
  ...
  defp raise_no_url do
    raise """
    Raxol.Earn.Onchain.RPC: no RPC URL configured. Set one of:

      config :raxol_earn, rpc: [url: "https://sepolia.base.org"]
      Raxol.Earn.Onchain.RPC.client(url: "...")
    """
  end
