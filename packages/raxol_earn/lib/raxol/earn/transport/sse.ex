defmodule Raxol.Earn.Transport.SSE do
  @moduledoc """
  Real `Raxol.Earn.Transport` against the Virtuals ACP Server-Sent
  ...
  Then in your agent:
      ...
      Raxol.Earn.Transport.connect(transport, %{
        owner: self(),
        chain_ids: [8453],
        wallet_address: "0x..."
      })
  ...
  - `:auth` -- `pid()` or name of a `Raxol.Earn.Auth` process.
  ...
  """
  @spec new(keyword()) :: Raxol.Earn.Transport.t()
  def new(opts) do
    table = :ets.new(:raxol_earn_transport_sse, [:set, :public])
  ...
  defp handle_chunk(owner, data) do
    ...
    {frames, rest} = Raxol.Earn.Transport.SSE.Parser.split_frames(accumulated)
    Process.put(:sse_buffer, rest)

    for frame <- frames do
      case Raxol.Earn.Transport.SSE.Parser.parse_frame(frame) do
