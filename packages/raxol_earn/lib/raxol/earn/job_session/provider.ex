defmodule Raxol.Earn.JobSession.Provider do
  @moduledoc """
  Drives a `Raxol.Earn.JobSession` from the PROVIDER side, tying together the
  three things a provider must do at each lifecycle point:

  1. invoke the offering's `Raxol.Earn.Offering.Handler` callback (via
     `Raxol.Earn.JobSession.HandlerSeam`) to make the accept / deliver /
     evaluate decision,
  2. write the corresponding hook call on-chain through
     `Raxol.Earn.HookClient` + the injected `Raxol.Earn.ProviderAdapter`, and
  ...
  This is the generic form of what `Raxol.Earn.Xochi.SolverAgent` does bespoke
  ...
  `Raxol.Earn.Checkpoint`), `deliver/2` therefore pins the encoded deliverable
  ...
  alias Raxol.Earn.{AssetToken, HookClient, JobSession}
  ...
  defp ck_key(p, step), do: Raxol.Earn.Checkpoint.key(p.chain_id, p.job_id, step)
