defmodule Raxol.Earn.ABI do
  ...
  Solidity primitives. Tuple/array types are not supported in v0.1.
      ...
      iex> Raxol.Earn.ABI.function_selector("transfer(address,uint256)")
      <<0xa9, 0x05, 0x9c, 0xbb>>
  """
  ...
  defp encode_static("uint8", value) when is_integer(value) and value in 0..255,
    do: encode_uint256(value)

  defp encode_static("uint192", value)
       when is_integer(value) and value in 0..0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF,
       do: encode_uint256(value)
  ...
  defp encode_static(type, _value) do
    raise ArgumentError, "Raxol.Earn.ABI: unsupported static type #{inspect(type)}"
  end
  ...
  defp encode_address(hex) when is_binary(hex) do
    case Base.decode16(hex, case: :mixed) do
      ...
      {:ok, bytes} ->
        raise ArgumentError, "Raxol.Earn.ABI: address must be 20 bytes, got #{byte_size(bytes)}"

      :error ->
        raise ArgumentError, "Raxol.Earn.ABI: invalid hex in address"
    end
  end
  ...
  defp encode_bytes32(hex) when is_binary(hex) do
    case Base.decode16(hex, case: :mixed) do
      ...
      {:ok, bytes} ->
        raise ArgumentError,
              "Raxol.Earn.ABI: bytes32 must be at most 32 bytes, got #{byte_size(bytes)}"

      :error ->
        raise ArgumentError, "Raxol.Earn.ABI: invalid hex in bytes32"
    end
  end
