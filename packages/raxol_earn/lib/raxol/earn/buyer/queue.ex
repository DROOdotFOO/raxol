defmodule Raxol.Earn.Buyer.Queue do
  @moduledoc """
  The buyer's dispatch authority -- the mirror of `Raxol.Earn.Seller.Queue`.

  Owns the client half of the ACP flow. `start_purchase/1` originates a job:
  it builds a `Raxol.Earn.JobSession.Client` from a purchase intent plus
  ...
  `dispatch/1` (called by `Raxol.Earn.Buyer.Runtime`) routes the job's later
  ...
  ## Configuration
      ...
      config :raxol_earn,
        ...
        buyer_provider_adapter: adapter,           # Raxol.Earn.ProviderAdapter (required to write)
        ...
        buyer_job_api_opts: [...],                 # Raxol.Earn.JobApi.HTTP.new/1 opts
        buyer_job_id_resolver: {mod, cfg}          # defaults to JobIdResolver.Receipt

  ## Telemetry

  - `[:raxol, :earn, :buyer, :queue, :purchased]` -- `%{job_id, offering}`.
  - `[:raxol, :earn, :buyer, :queue, :dispatched]` -- `%{type, job_id}`.
  - `[:raxol, :earn, :buyer, :queue, :dropped]` -- `%{type, job_id, reason}`.
  """

  use Raxol.Core.Behaviours.BaseManager

  alias Raxol.Earn.{Chain, JobApi}
  ...
  @doc """
  Re-track an already-created job (used by `Raxol.Earn.Buyer.Resync` on restart).
  ...
  defp read_defaults do
    %{
      buyer_address: Application.get_env(:raxol_earn, :buyer_address),
      provider_adapter: Application.get_env(:raxol_earn, :buyer_provider_adapter),
      checkpoint: Raxol.Earn.Checkpoint.store(),
      chain_id: Application.get_env(:raxol_earn, :buyer_chain_id, 8453),
      acp_core_address:
        Application.get_env(:raxol_earn, :buyer_acp_core_address) ||
          Chain.mainnet().acp_core_address,
      api: buyer_job_api(),
      resolver: buyer_resolver(),
      ledger: Application.get_env(:raxol_earn, :buyer_ledger),
      policy: Application.get_env(:raxol_earn, :buyer_spending_policy),
      agent_id: Application.get_env(:raxol_earn, :buyer_agent_id, :raxol_buyer)
    }
  end
  ...
  defp buyer_resolver do
    case Application.get_env(:raxol_earn, :buyer_job_id_resolver) do
      nil -> %{adapter: Raxol.Earn.JobIdResolver.Receipt}
  ...
  defp emit(suffix, metadata) do
    :telemetry.execute([:raxol, :earn, :buyer, :queue, suffix], %{}, metadata)
  end
end
