defmodule Raxol.Earn.Buyer.Supervisor do
  @moduledoc """
  Supervisor for the buyer half of the ACP package -- the mirror of
  `Raxol.Earn.Seller.Supervisor`.
  ...
  2. `Raxol.Earn.Buyer.Queue` -- the dispatch authority. Must outlive the Runtime
     so in-flight purchases finish even if the backend reconnect cycles.
  3. `Raxol.Earn.Buyer.Resync` -- drain-before-act: rehydrates in-flight jobs from
     the job API before live dispatch. Inert without `:buyer_job_api_opts`.
  4. `Raxol.Earn.Buyer.Runtime` -- subscribes to the buyer backend and forwards
  ...
  Started by `Raxol.Earn.Supervisor` only when
  `config :raxol_earn, buyer_enabled: true`. Seller-only users pay nothing for the
  buyer machinery.
  """
  ...
  defp checkpoint_owner_children do
    ets? = match?({:ets, _name}, Application.get_env(:raxol_earn, :checkpoint))
    seller_owns? = Application.get_env(:raxol_earn, :seller_enabled, false)

    if ets? and not seller_owns?, do: [Raxol.Earn.Checkpoint.Owner], else: []
  end
end
