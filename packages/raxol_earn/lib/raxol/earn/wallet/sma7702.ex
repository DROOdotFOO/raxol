defmodule Raxol.Earn.Wallet.Sma7702 do
  @moduledoc """
  ...
  Unlike `Raxol.Earn.Wallet.SCA` -- which targets a DEPLOYED Modular Account v2
  ...
  in two ways:
    ...
    * **Replay-safe digest domain.** The account wraps the app digest with its
      ...
      `{module, salt = account}` domain that `Raxol.Earn.Wallet.SCA` builds.
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      def sign_message(message) do
        Raxol.Earn.Wallet.Sma7702.sign_message(
          message,
          unquote(provider),
          unquote(chain),
          unquote(account)
      ...
      def sign_typed_data(domain, types, message) do
        Raxol.Earn.Wallet.Sma7702.sign_typed_data(
          domain,
          types,
          message,
          unquote(provider),
  ...
  # `Raxol.Earn.Wallet.SCA`'s message hashing).
