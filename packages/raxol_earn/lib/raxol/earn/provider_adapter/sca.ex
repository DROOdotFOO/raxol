defmodule Raxol.Earn.ProviderAdapter.SCA do
  @moduledoc """
  Production `Raxol.Earn.ProviderAdapter` backed by an Elixir-native
  Alchemy Modular Account v2 smart account (`Raxol.Earn.Wallet.SCA`).
  ...
  to the chain's JSON-RPC via `Raxol.Earn.Onchain.RPC`; they never touch a
  key.

  ## Construction

      adapter =
        Raxol.Earn.ProviderAdapter.SCA.new(
          wallet: MyAgent.SCA,               # a `use Raxol.Earn.Wallet.SCA` module
  ...
  `config :raxol_earn, rpc: [url: ..., plug: ...]`.

  ## Provisioning

  On the first write, if `auto_provision` is true (default), the adapter
  runs `Raxol.Earn.Wallet.SCA.Provisioner.ensure_provisioned/3` **before**
  ...
  caller needs `executeBatch` (`Raxol.Earn.HookClient.submit_single/5`
  ...
  alias Raxol.Earn.Wallet.SCA.{EntryPoint, ModularAccount, Provisioner, UserOp}
  ...
  - `:wallet` -- a `use Raxol.Earn.Wallet.SCA` module.
  ...
  defp validate_wallet!(wallet) do
    ...
    if not (function_exported?(wallet, :send_sponsored_user_operation, 2) and
      ...
      raise ArgumentError,
            ":wallet must be a Raxol.Earn.Wallet.SCA module " <>
  ...
  # the chains map; a test stub plug is picked up from the :raxol_earn :rpc
