defmodule Raxol.Earn.ProviderAdapter.JSONRPC do
  @moduledoc """
  Production `Raxol.Earn.ProviderAdapter` backed by raw Ethereum
  JSON-RPC and an EOA private key.

  Reuses `Raxol.Earn.Onchain.RPC` for the wire layer and
  `Raxol.Earn.Onchain.Transaction` for EIP-1559 transaction
  ...
  Nonce assignment is serialized through a `Raxol.Earn.Wallet.NonceServer`
  ...
  - **Smart-contract account flows**: this adapter signs EIP-1559
    ...
    flows, use `Raxol.Earn.ProviderAdapter.SCA`, which wraps
    `Raxol.Earn.Wallet.SCA` and uses ERC-4337 EntryPoint nonces instead.
  ...
  alias Raxol.Earn.{ABI, Onchain.Hex, Onchain.RPC, Onchain.Transaction, Secret}
  ...
  - `:nonce_server` -- the `Raxol.Earn.Wallet.NonceServer` instance that
    serializes nonce assignment for THIS adapter's wallet address
    (default `Raxol.Earn.Wallet.NonceServer`, the umbrella instance the
