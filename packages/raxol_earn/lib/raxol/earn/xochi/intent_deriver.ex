defmodule Raxol.Earn.Xochi.IntentDeriver do
  @moduledoc """
  Read the authoritative Xochi intent behind a job's `signed_intent`.

  Both storefront drivers -- the generic `Raxol.Earn.JobSession.Provider` path
  (via `UsdcPublicOffering.resolve_accept/2`) and the bespoke
  `Raxol.Earn.Xochi.SolverAgent` -- must size the storefront fee on the amount the
