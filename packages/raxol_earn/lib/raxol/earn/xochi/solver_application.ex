defmodule Raxol.Earn.Xochi.SolverApplication do
  @moduledoc """
  ...
  everything downstream):
    ...
    1. `Raxol.Earn.Auth` -- EIP-712 auth against the Virtuals server.
    2. `Raxol.Earn.Agent` -- REST (`JobApi.HTTP`) + SSE (`Transport.SSE`) job
       orchestration, role `:provider`.
    3. `Raxol.Earn.Xochi.SolverAgent` -- per-job driver; on `job.funded` it runs the
    ...
    5. `Raxol.Earn.Xochi.Heartbeat` -- periodic liveness log + telemetry, last so its
       crash restarts nothing above it.

  The `settle_fn` is a pure relay (`Raxol.Earn.Xochi.Settler.build/1`, `:xochi_config`
  ...
  Opt-in, mirroring `:seller_enabled`. Never starts under `MIX_ENV=test`.
      ...
      config :raxol_earn, xochi_solver_enabled: true

  `RaxolEarn.Application` starts it when the flag is set; an operator embedding raxol
  may instead add `Raxol.Earn.Xochi.SolverApplication` to their own supervision tree.
  ...
  alias Raxol.Earn
  alias Raxol.Earn.Xochi.{Heartbeat, Settler, SolverAgent}
  ...
  @doc """
  Whether the solver runtime should auto-start. Off by default; `RaxolEarn.Application`
  only runs outside `MIX_ENV=test` (the package sets `:mod` only for non-test), so
  this flag alone gates auto-start.
  """
  @spec enabled?() :: boolean()
  def enabled?, do: Application.get_env(:raxol_earn, :xochi_solver_enabled, false)

  @impl true
  def init(_opts) do
    chain = Earn.Chain.mainnet()
    ...
    api = Earn.JobApi.HTTP.new(auth: auth_name(), server_url: server_url, chain_ids: [@chain_id])
    transport = Earn.Transport.SSE.new(auth: auth_name(), server_url: server_url)
    ...
    children =
      sidecar_children ++
        [
          ...
          %{
            id: :stream_starter,
            start: {Task, :start_link, [fn -> :ok = Earn.Agent.start_stream(agent_name()) end]},
            restart: :transient
          },
  ...
  defp build_provider(chain) do
    ...
    if privy_mode?() do
      ...
      provider =
        Earn.ProviderAdapter.Privy.new(
      ...
      {provider, address, [{Raxol.Earn.SignerSidecar, []}]}
    else
      ...
      # `Raxol.Earn.Secret` before it enters the provider config. That wrapper redacts under
      ...
      provider =
        Earn.ProviderAdapter.JSONRPC.new(
          chains: %{@chain_id => rpc_url},
          private_key: private_key
        )

      {provider, Earn.ProviderAdapter.get_address(provider), []}
    end
  end
