defmodule Raxol.Earn.JobApi.Mock do
  @moduledoc """
  In-process mock for `Raxol.Earn.JobApi`. Tests pre-seed agents,
  active jobs, and a `getMe` response; this module returns them
  without touching the network.

  ## Usage

      api = Raxol.Earn.JobApi.Mock.new(me: %{wallet_address: "0x...", name: "Xochi"})
      Raxol.Earn.JobApi.Mock.put_agent(api, "0xabc...", %{name: "Other"})
      Raxol.Earn.JobApi.Mock.put_active_jobs(api, [%{job_id: "1", chain_id: 8453}])
      {:ok, [agent]} = Raxol.Earn.JobApi.browse_agents(api, "Other", %{})
  ...
  def new(opts \\ []) do
    table = :ets.new(:raxol_earn_jobapi_mock, [:set, :public])
  ...
  @spec put_agent(Raxol.Earn.JobApi.t(), String.t(), Raxol.Earn.JobApi.agent_detail()) :: :ok
  ...
  @spec put_active_jobs(Raxol.Earn.JobApi.t(), [map()]) :: :ok
