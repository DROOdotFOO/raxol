...
defmodule Raxol.AgentClientProtocol.ErrorTest do
  ...
  test "error codes" do
    ...
    assert Error.auth_required_code() == -32_000
  ...
  test "to_json/from_json round-trip" do
    err = Error.new(-32_600, "Invalid request")
    json = Error.to_json(err)
    assert json == %{"code" => -32_600, "message" => "Invalid request"}
  ...
  test "code_name" do
    assert Error.code_name(-32_700) == :parse_error
    assert Error.code_name(-32_601) == :method_not_found
  ...
  test "from_json/1 is total: missing required fields never raise" do
    ...
    assert {:error, :invalid_error} = Error.from_json(%{"code" => -32_000})
  ...
  test "nested inside a struct, Jason.encode! never crashes on unknown-field errors" do
    {:ok, err} = Error.from_json(%{"code" => -32_602, "message" => "bad", "x-trace" => "abc"})
    encoded = Jason.encode!(err)
    decoded = Jason.decode!(encoded)
