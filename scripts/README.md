...
| `worktree.sh` | Adds a private 0700 parallel git worktree with transactionally cloned `deps`/`_build` caches. Workstation-only: seeding skips Hex verification, rejects symlinked seed paths, checks lock and path-dependency manifests, and `--fresh` opts out |
