...
- **Security: `Raxol.System.Updater` installed downloads it never verified, from a repository it did not own.** The self-updater was pinned to `username/raxol` (and `DeltaUpdater` to `raxol/raxol`), so whoever controlled those GitHub accounts decided what `self_update/2` downloaded. It unpacked archives with the host's `tar`/`unzip` and copied the result over the running executable; the `verify_checksums: true` setting was never read. The delta path ran the patched binary (`--version`) before checking anything, and `Core` passed `apply_delta_update/2` its arguments in reverse order. Now:
  ...
  - Each download must match its `SHA256SUMS` entry before it is extracted, patched, or installed. A missing, duplicated or malformed entry means nothing is downloaded at all.
  ...
  - A delta's own checksum is verified before `bspatch` runs, and the patched output must hash to the full binary's published checksum. Nothing is executed to check it.
...
- **`Raxol.System.DeltaUpdaterSystemAdapterBehaviour` / `DeltaUpdaterSystemAdapterImpl`**, the `verify_checksums` update setting (verification is no longer optional), and `Raxol.System.Updater.Validation.get_platform/0` (see `Manifest.host_platform/0`). `DeltaUpdater.apply_delta_update/2` now takes `(target_version, opts)`, and the `Updater` functions take an options keyword (documented on the module).
