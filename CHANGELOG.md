...
- **Quitting a full-screen app: Ctrl+C opened the BEAM BREAK menu, shutdown logs printed after quit, and a child crashing in `terminate/2` aborted the teardown (#1128).** (1) `raxol_terminal`: the Terminal Driver ran `Stty.raw!/0` before `start_stdin_reader/1`, whose prim_tty reinit writes prim_tty's own raw mode, the termios saved at VM boot minus ICANON and ECHO. ISIG came back on, so ^C raised SIGINT and the node hung in the BREAK menu with the terminal still in the alternate screen. Raw mode is now applied after the reinit, so ^C reaches the app as `%{key: :char, char: "c", ctrl: true}`; the `mix raxol.new` templates and the examples already bind it to `Directive.stop()`, and it now quits with exit 0 and the terminal restored. The runtime still does not quit on Ctrl+C itself: `update/2` has no "unhandled" result to fall back on, and apps own the key. (2) The Driver's cleanup set the Logger level to `:debug` instead of the level it found at init; it now restores that level. The Lifecycle stops the Driver first, so everything the rest of its teardown logged (about 7 `[info]`/`[debug]` lines) printed on the restored terminal. When the session ran with Logger at `:none`, `Lifecycle.terminate/2` now drops log events until the teardown is done (`Shutdown.quietly/2`, a primary `:logger` filter). (3) `Shutdown.stop_process/2` used `rescue`, which does not catch the exit from `GenServer.stop/3` when the process crashes in `terminate/2`, so the Lifecycle's teardown stopped at that child and the Lifecycle died with the crash, taking down its `start_link` caller. The exit is now caught and logged with the child's label, pid and reason, and the teardown continues. New tests failed before the fix: a real-pty `-isig` test for the full-screen Driver (^C never decoded, `isig_off=false`), the Logger level after `cleanup_terminal/1` (`:none`), the Lifecycle's teardown log (six lines), and a Lifecycle whose driver raises in `terminate/2` (the linked test process was killed).
