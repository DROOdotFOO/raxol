...
- **`Raxol.Core.Metrics.AlertManager` honours its `check_interval` option (#1106).** `schedule_check` always used the 60-second default, so a manager started with a shorter or longer interval still checked once a minute. The option (in seconds, as documented) is now used; the test helper that passed `:timer.seconds(1)`, i.e. 1000 s, now passes `1`. The new test waited 3 s for a check at `check_interval: 1` and got none before the fix.
