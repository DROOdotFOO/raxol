...
- **Security: anonymous code execution through the playground REPL demo (#1045).** The HTTPS gallery serves every catalog entry at `/demos/:demo` with no auth, and `Raxol.Playground.Demos.ReplDemo` evaluated whatever a browser typed. Two fixes: `Raxol.REPL.Sandbox` now refuses `import`, `alias`, `require` and `use` at `:standard` and `:strict` (`import System; cmd("id", [])` and `alias :os, as: Enum; Enum.cmd(~c"id")` both returned `:ok` before, because every other clause decides safety from a module name and those four forms decide which module a name reaches), and the demo no longer evaluates at all unless the deployment sets `RAXOL_REPL_EXPOSED=true` (or `config :raxol, :repl_exposed, true`), the same flag `Raxol.Payments.Deployment.assert_signing_isolated!/0` reads. Sandboxed code that names its modules in full is unaffected; `:none`, the local-terminal level, is unchanged.
