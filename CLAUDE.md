...
**REPL Architecture**: `Raxol.REPL.Evaluator` wraps `Code.eval_string` with a monitored spawn plus timeout, `Raxol.REPL.CaptureIO` IO capture via group_leader swap, and persistent bindings across evaluations. It is NOT a sandbox: `base_env/0` is an unrestricted `Macro.Env`, and the timeout/heap/result/output caps bound the one evaluation process, not what it spawns or reaches (filesystem, `:os.cmd`, ports, distribution). See its "Trust boundary" moduledoc section. `Raxol.REPL.Sandbox` is a separate check a caller runs first; it scans ASTs via `Macro.prewalk` at three levels: `:none` (unrestricted), `:standard` (blocks destructive calls plus process creation, scheduling, and module indirection), `:strict` (whitelist-only and the default for every launcher; the whole of the boundary there). `Evaluator.with_vfs/1` seeds a VFS binding and auto-imports `Raxol.REPL.VfsHelpers` via prelude.
