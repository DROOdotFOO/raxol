...
**Agent Commerce Protocol** (in `packages/raxol_acp/`, pre-alpha): Elixir/OTP-native implementation of the Virtuals ACP for selling agent services on Base, canonically aligned to the real deployed contracts and live-validated against a Base fork. One supervised `Raxol.ACP.Job.Server` per active job (mirrors `Raxol.Agent.Session`: Registry lookup + DynamicSupervisor + transient restart). `Raxol.ACP.Job.StateMachine` is a pure module; states are `:request -> :negotiation -> :transaction -> :evaluation -> :completed` plus `:rejected` (seller rejects from `:request`, phase id 5) and `:expired`. ACP has NO off-chain memo signing. Memos are on-chain `createMemo` calls; `Job.MemoType` (0..9) + `Job.FeeType` (0..3) enums match acp-node (the speculative `Job.Memo` EIP-712 module was deleted). `Raxol.ACP.Wallet.NonceServer` serializes EVM nonce assignment via the GenServer mailbox (the integration plan claimed process-per-job avoided concurrent-Alchemy collisions; it doesn't, this does). `Raxol.ACP.ContractClient` behaviour matches the real `ACPSimple`/`ACPRouter` write surface (`create_job(provider,evaluator,expired_at)`, `set_budget`, `set_budget_with_payment_token`, `create_memo/5`, `create_payable_memo`, `sign_memo/3`, `claim_budget`, `confirm_x402_payment_received`, `withdraw_escrowed_funds` for buyer escrow reclaim after non-delivery); two impls: `InMemory` (tests) and `Onchain` (Req JSON-RPC + EIP-1559 typed-tx signing + Yellow-Paper RLP). `Onchain.create_job` resolves job_id from the `JobCreated` non-indexed `data` word (`:create_job_id_source`/`:create_job_event_signature`, `LogDecoder.extract_data`) and fails closed on an unresolved id (opt back into the tx-hash placeholder via `allow_placeholder_job_id`); a lost receipt returns `{:receipt_pending, tx_hash, _}` so a retry re-queries, a pre-broadcast failure rolls the nonce back, and token amounts scale by `:token_decimals` (default 6). `Job.Server` takes an optional `:expired_at` that arms an auto-expire timer; `Job.Server.reclaim/1` calls `withdraw_escrowed_funds`. `:acp_version` config (`:v1` default | `:v2`) version-gates `create_job`/`create_payable_memo`; all other methods are byte-identical. Real ABIs vendored at `priv/abi/{acp_simple,acp_router}.json`; Blockscout-verified Base addresses + `acp_socket_url` wired into `Raxol.ACP.Chain`. `Raxol.ACP.Wallet.SCA` is a full ERC-4337 v0.7 / Alchemy Modular Account v2 stack (`UserOp` hash viem-verified, `Bundler`, `ModularAccount`, `Paymaster`, `Provisioning` for counterfactual CREATE2 + session-key `installValidation`); `Onchain` detects an SCA wallet by capability and routes writes through sponsored UserOps, self-deploying on the first tx. `Raxol.ACP.ABI` is a hand-rolled Solidity encoder. `Raxol.ACP.Offering` DSL: `use Raxol.ACP.Offering, name:, price_usdc:, sla_minutes:, cluster:` injects the `Handler` behaviour and registers metadata; the `Registry` (ETS) stores offering specs. Handler integration in `Job.Server.accept_request/1` and `deliver/1` auto-invokes the configured handler module + auto-signs via the configured wallet. Self-starts via `RaxolAcp.Application` outside `:test`. Depends on raxol_payments at runtime, raxol_mcp compile-time only. Shipped: `Onchain` SCA routing, Seller stack (`Backend.{InMemory, WebSocket}`; the WebSocket backend speaks Socket.IO v4/Engine.IO over `Mint.WebSocket`; `Queue` + `Runtime` + `Supervisor`, opt-in via `:seller_enabled`), `Job.Store` ETS+optional-DETS persistence, and `mix raxol_acp.bench`. Live-validated: a `:live_chain` harness drives the EOA pipeline against local anvil; a `:live_bundler` harness forks Base and submits a UserOp via the real on-chain `EntryPoint.handleOps` (RAN GREEN). Both live tags need foundry (`anvil`/`cast`) and are excluded by default. A v2 agent stack mirroring `acp-node-v2` also lands: `Raxol.ACP.Agent` is the orchestrator wiring three injected behaviours -- `ProviderAdapter` (low-level EVM provider: `sendCalls`/`signMessage`/`signTypedData`/`getLogs`, with `InMemory` + `JSONRPC` impls), `Transport` (SSE chat stream, `Mock` + `SSE`), and `JobApi` (off-chain REST discovery, `Mock` + `HTTP`). `Raxol.ACP.JobSession` replaces v1's `Job.Server` for the v2 model: one GenServer per job registered by `{chain_id, job_id}`, role-aware status, chronological entry log, subscriber notifications via `{Raxol.ACP.JobSession, {chain_id, job_id}, entry}`.
