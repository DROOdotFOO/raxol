...
- `RAXOL_REBALANCE_DEMAND_MULTIPLIER` / `RAXOL_REBALANCE_DEMAND_FLOOR_CAP` - Demand-aware solver inventory floors (`peak * multiplier`, capped). One setting, enforced over the RESULTING pair rather than over which key a caller supplied: set both or set neither, and either alone raises at boot, since `peak` comes from orders anyone can place. Set-but-empty counts as unset, so clearing a configured deployment means clearing both. `RAXOL_REBALANCE_DEMAND_WINDOW_MS` (default `86400000`) is how far back demand is read. Full contract in `Raxol.Payments.Accounting`
