...
| `raxol_console` loader | Package scaffolded (compiles against the full dep chain). `RuntimeConfig.build/2` (pure package + env -> config: persona, scheduler jobs, bundled MCP servers) + graduated `Scheduler.Wiring` done. Remaining: `Boot`/`Supervisor`/`Application` (start agent + gateway subtrees, reconcile scheduler jobs) + `Console.Bench.Adapter` | M | In progress |
