...
| `raxol_console` loader | Package scaffolded. `RuntimeConfig.build/2` + `Scheduler.Wiring` + `Boot`/`Supervisor`/`Reconciler` (starts the persona-wired scheduler and idempotently converges `tasks.json` jobs) done. Remaining: gateway channel subtree + MCP-into-handler wiring, `Console.Application` entrypoint, `Console.Bench.Adapter` | M | In progress |
