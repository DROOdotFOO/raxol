defmodule Raxol.UI.Layout.FlexItem do
  ...
  """
  def resolve(child, main_axis, container, content_size_fun, auto_min_fun \\ fn -> 0 end) do
    ...
    explicit_main = resolve_dimension(axis_style(style, child, main_dim), container_main)
    ...
    cross_size =
      case resolve_dimension(axis_style(style, child, cross_dim), container_cross) do
        :auto -> nil
        n -> n
      end
  ...
  def hypothetical_main(%__MODULE__{} = item), do: clamp_main(item, item.base_size)
  ...
  def cross_margins(%__MODULE__{margin: {t, _r, b, _l}}, :horizontal), do: {t, b}
  ...
  defp read_explicit_flex_keys(style, defaults) do
    %{
      ...
      | grow: clamp_factor(Map.get(style, :flex_grow, defaults.grow), :grow),
        shrink: clamp_factor(Map.get(style, :flex_shrink, defaults.shrink), :shrink),
  ...
  defp resolve_margin(child, style, container_width) do
    ...
    normalized =
      case raw do
        n when is_integer(n) or n == :auto -> {n, n, n, n}
        {h, v} -> {h, v, h, v}
        {_t, _r, _b, _l} = m -> m
        other ->
