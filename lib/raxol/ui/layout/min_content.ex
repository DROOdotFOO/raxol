defmodule Raxol.UI.Layout.MinContent do
  ...
  def width(%{type: type} = el) when type in [:row, :column] do
    width(Map.put(el, :type, :flex) |> Map.put_new(:direction, container_direction(type)))
  end

  def width(%{type: :view} = el), do: children_min(children_of(el), :max)

  # Components that render a fixed-ish chrome around text
  def width(%{type: :button} = el), do: TextMeasure.display_width(text_of(el)) + 4
  def width(%{type: :checkbox} = el), do: TextMeasure.display_width(text_of(el)) + 4
