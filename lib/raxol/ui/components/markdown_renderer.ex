defmodule Raxol.UI.Components.MarkdownRenderer do
  ...
  Inline formatting (bold/italic/code/links) is rendered as styled spans
  (a `:row` of individually-styled `text` elements) whenever the fully
  formatted line fits within `width`. When it does not fit, the line is
  word-wrapped via `Raxol.UI.TextLayout.wrap/3` and rendered as plain,
  unstyled text -- span-aware wrapping (preserving styles across a wrap
  boundary) is not implemented and is left as future work. Wrapped output
  never leaves literal Markdown marker characters (`*`, `_`, backticks) in
  the text; markers are always consumed by the segment parser before
  wrapping happens.
  """
  ...
  #
  # Given `[{text, style}, ...]` segments (from either the Earmark AST or
  # the built-in regex parser), renders them as:
  #
  #   * a single plain `text` element, when every segment is unstyled
  #     (preserves the pre-existing single-element output shape for lines
  #     with no inline formatting at all);
  #   * a `:row` of individually-styled `text` spans, when at least one
  #     segment is styled AND the full line (prefix + all segment text)
  #     fits within `width`;
  #   * one or more plain, unstyled `text` elements (word-wrapped via
  #     `Raxol.UI.TextLayout.wrap/3`), when the full line does not fit.
  #     Wrapping operates on the plain concatenated segment text (not the
  #     prefix); the prefix is re-attached to only the first wrapped line.
  #     This is a known limitation -- styling does not survive a wrap
  #     boundary -- but no marker characters ever leak into the output,
  #     since markers were already consumed by segment parsing.
  ...
  defp wrap_plain_lines(prefix, text, width) do
    case TextLayout.wrap(text, width, :normal) do
      [] ->
      ...
      [first | rest] ->
        [
          ...
          | Enum.map(rest, &Components.text(content: &1))
        ]
    end
  end
  ...
  # `builtin_segments/1` parses a plain (marker-bearing) line into
  # `[{text, style}]` segments, mirroring `inline_segments/1` for the
  # Earmark AST path. Precedence (encoded via alternation order, tried
  # left-to-right at each match position): code span first, then
  # `**bold**`/`__bold__`, then `*em*`/`_em_`, then `[text](url)` links.
  # Unmatched/unbalanced markers never match the pattern, so they pass
  # through untouched as literal text (e.g. a lone `*` stays a `*`).
