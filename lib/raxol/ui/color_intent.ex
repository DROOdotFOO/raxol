defmodule Raxol.UI.ColorIntent do
  ...
  See `docs/proposals/in-flight/region-prominence-propagation.md` §3.1 for
  the full design rationale. This module is the struct only — no resolution
  ...
  ## Fields
    ...
    * `:floor` - the legibility floor class: `:none` (pure fade, the
      ...
      `:text` (WCAG AA normal text, `4.5`, ratified), or `{:ratio, float()}`
  ...
  unset, `prominence` fades from the **baseline**-tier target (see the
  design doc §3.1 "Rules"). A bare `%ColorIntent{}` therefore resolves as a
  baseline-tier neutral at full prominence.
  ...
  through unchanged today (Phase 0 -- no producer emits intents yet, so
  every real render is 100% literal, and the resolver is byte-identical to
  no-resolver by construction).
  """
