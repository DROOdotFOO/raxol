defmodule Raxol.Playground.App do
  ...
  @box_border_lines 2
  # pale pastel blue, low-intensity so the scrollbar thumb reads as a
  # subtle wash rather than a bright highlight against a dark terminal
  ...
  defp main_view(model) do
    ...
    # Grow the content row so it absorbs all vertical space between the
    # header and the status bar; the status bar stays pinned to the bottom
    # of the terminal instead of floating under the content.
  ...
  # Windowing is keyed on items alone (never on rendered lines): category
  # header rows are decoration, and threading them through the cursor
  # index would let a header crossing inflate a single navigation step
  # into a 2-row scroll jump, breaking the "scroll by exactly one" edge
  # contract (see ScrollWindow's step-invariant property tests).
  #
  # So: while the full list (items + their headers) fits in the budget,
  # render it exactly as before -- every item, every header, no
  # scrollbar. Once it doesn't, drop headers and switch to a flat,
  # windowed item list with the proportional scrollbar; the box then
  # never grows past its available height.
