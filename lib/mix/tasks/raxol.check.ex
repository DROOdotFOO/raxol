defmodule Mix.Tasks.Raxol.Check do
  ...
  defp run_check(:rate) do
    Mix.shell().info(Colors.subsection_header("RATE golden render"))

    case Mix.shell().cmd("mix raxol.rate") do
      0 ->
        Mix.shell().info(
        ...
        Mix.shell().error(
          "    " <>
            Colors.format_error(
              ...
              "inspect the diff, then run `mix raxol.rate --gen` if the change is intended"
            )
        )
