...
Blockscout appears in this repo only in four doc comments, all of the form "verified on-chain
via Blockscout". Etherscan appears only as a map of URL prefixes for building human-readable
links in a Mix task (`raxol_earn.order.ex`), and that map covers neither chain 4663 nor Tron.
...
- **Downstream we are a modern-only server.** The public surface is genuinely stateless, so
  any node answers any request with no session affinity and no shared session store. For a
  free public service on the BEAM this is close to ideal.
...
```
raxol_mcp      --> raxol_core                      (gains Aggregator)
raxol_web3     --> raxol_core, req                 (raxol_mcp optional)
raxol_payments --> raxol_web3
raxol_earn     --> raxol_payments, raxol_web3
```

Three modules move down, unchanged in behaviour, with deprecated delegating shims left in
...
The contract is **six required callbacks and two optional**, not the eight first drafted.
Surveying what non-EVM chains can actually answer (recorded in the companion survey) demoted
two of them:
...
`raw_request/2` as the escape hatch for endpoints the contract does not model.
...
with per-backend health gated by `Raxol.MCP.CircuitBreaker`.
...
Every tool in this package is a read and carries no `destructiveHint` or `sensitive`
annotation, so the server runs under a nil authorizer on stdio. This is a deliberate
constraint rather than an oversight: `Raxol.MCP.Server` refuses to boot when any registered
tool is sensitive and no `:authorizer` is configured, so admitting a single write tool would
force an authorizer onto the whole server and change the deployment story for every consumer.
Writes stay where they already are, behind the spend-gated Actions in `raxol_payments`.
...
| ----- | ------------------- | -------- | -------- | ------------------ |
| EVM 1, 10, 137, 8453, 42161 | Blockscout REST `/api/v2/*` | eth and arbitrum returned 200; base and polygon returned 500; optimism redirects | Etherscan V2, then raw RPC | Cloudflare rejects non-browser User-Agents with a 403 challenge page, so the HTTP client must set a realistic one |
...
### 6. Reuse rather than reinvention

A free-tier aggregator is constrained by rate limits, caching, and clean failover. All four
primitives already exist in the repo:

| Need | Module | Notes |
| ---- | ------ | ----- |
| Response cache | `Raxol.Agent.Cache` | A behaviour with ETS and Postgrex adapters already written, so a hosted instance and a local one differ by config alone |
| Free-tier budgets | `Raxol.RateLimit` | `check/3` returning `:ok` or `{:error, :rate_limited}`, plus `configure/2` and `reset_in/3` |
| Backend health | `Raxol.MCP.CircuitBreaker` | ETS-backed `check/3`, `record_success/2`, `record_failure/3` |
| Confirmation polling | `Raxol.Payments.Poll` | `run/3` with a wall-clock budget |
| Upstream MCP clients | `Raxol.MCP.Client` | Spawn, handshake, `list_tools/2`, `call_tool/4`, `tool_name/2` |

Rate limiting must be a token bucket rather than retry-on-429, because TronGrid's 30-second
burst penalty turns naive retry logic into a sustained outage.
...
- No single indexer is load-bearing. Each chain has a primary and at least one fallback, and
  the router can fail over on health.
...
- A fourth package in the payments dependency chain, and three module moves that touch
  `raxol_payments` and `raxol_earn` call sites.
...
- The `raw_request/2` passthrough shape per backend.
...
Two claim classes carry this ADR, and both are reproducible.
...
with a browser User-Agent, and `api.cantonnodes.com/v0/round-of-latest-data`.
...
- `docs/proposals/web3-upstream-survey.md`: the per-upstream PROXY, PORT, and AVOID verdicts,
  the wire-level findings behind the concurrency policy, and the evidence for the six-plus-two
  contract
...
- `docs/PACKAGES.md` for the canonical dependency graph
