...
| D2 | `flex: 1` semantics | DECIDED: terminal-pragmatic — `flex: 1` sugar sets `min_main: 0`, always equalizes; documented as intentional CSS divergence |
| D3 | `:view` stays passthrough | DECIDED: yes |
| D4 | `:row`/`:column` merge: explicit default-compat map, Phase D only | DECIDED: yes |
| D5 | Third stack (`Core.Renderer.View.Layout.Flex`, 566 LOC) | DECIDED: zero production callers — only `view_test.exs` + `test/support/raxol/visual.ex:96` (visual snapshot harness renders through the DEAD pipeline — snapshots certify a path production never runs). Re-point `visual.ex` to `UI.Layout.Engine`, re-pin snapshots consciously, delete stack + its tests in Phase D |
...
### Phase E — text layout (independent after B)
...
### Phase F — overflow + scroll anchoring (needs design pass)
